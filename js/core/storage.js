...
const IDB_STORE_STUDYPLAN_PDFS = 'studyPlanPdfs';
...
function openIDB() {
  ...
  _idbPromise = new Promise((resolve) => {
    ...
    req.onupgradeneeded = e => {
      ...
      if (!db.objectStoreNames.contains(IDB_STORE_STUDYPLAN_PDFS)) db.createObjectStore(IDB_STORE_STUDYPLAN_PDFS, { keyPath: 'id' });
...
async function idbGetAllStudyPlanPdfs() {
  const db = await openIDB();
  return new Promise((resolve, reject) => {
    const tx = db.transaction(IDB_STORE_STUDYPLAN_PDFS, 'readonly');
    const req = tx.objectStore(IDB_STORE_STUDYPLAN_PDFS).getAll();
    req.onsuccess = () => resolve(req.result || []);
    req.onerror = () => reject(req.error);
  });
}
async function idbPutStudyPlanPdf(entry) {
  const db = await openIDB();
  return new Promise((resolve, reject) => {
    const tx = db.transaction(IDB_STORE_STUDYPLAN_PDFS, 'readwrite');
    tx.objectStore(IDB_STORE_STUDYPLAN_PDFS).put(entry);
    tx.oncomplete = () => resolve(true);
    tx.onerror = () => reject(tx.error);
  });
}
async function idbDeleteStudyPlanPdf(id) {
  const db = await openIDB();
  return new Promise((resolve, reject) => {
    const tx = db.transaction(IDB_STORE_STUDYPLAN_PDFS, 'readwrite');
    tx.objectStore(IDB_STORE_STUDYPLAN_PDFS).delete(id);
    tx.oncomplete = () => resolve(true);
    tx.onerror = () => reject(tx.error);
  });
}
async function idbClearStudyPlanPdfs() {
  const db = await openIDB();
  return new Promise((resolve, reject) => {
    const tx = db.transaction(IDB_STORE_STUDYPLAN_PDFS, 'readwrite');
    tx.objectStore(IDB_STORE_STUDYPLAN_PDFS).clear();
    tx.oncomplete = () => resolve(true);
    tx.onerror = () => reject(tx.error);
  });
}
