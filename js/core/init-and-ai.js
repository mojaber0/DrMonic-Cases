...
تصرّف كطبيب ومعلم مبسّط لطلاب الطب: اشرح بالعربية الواضحة وعلّم الطالب طريقة التفكير السريري.
جاوب فقط على السؤال يلي يطرحه الطالب مباشرة — أنت لا تشوف الصفحة اللي هو فاتحها ولا محتواها، فلو سؤاله ناقص تفاصيل، اسأله يوضح بدل ما تفترض سياق مش موجود عندك.
...
function aiAssistantSaveApiKey() {
  const input = document.getElementById('aiAssistantApiKeyAdmin');
  const status = document.getElementById('aiAssistantAdminKeyStatus');
  const setStatus = t => { if (status) status.textContent = t; };
  const key = (input?.value || '').trim();
  if (!key) {
    localStorage.removeItem(AI_ASSISTANT_API_KEY_STORAGE_KEY);
    setStatus('🗑️ تم مسح مفتاح Gemini من هذا المتصفح.');
    if (input) input.placeholder = 'Gemini API Key';
    return;
  }
  if (!(key.startsWith('AIza') || key.startsWith('AQ.'))) {
    setStatus('⚠️ تنبيه: مفاتيح Gemini تكون غالباً إما AIza للمفاتيح القديمة أو AQ. للمفاتيح الجديدة من AI Studio. المفتاح محفوظ، لكن إذا فشل الاتصال تأكد منه من AI Studio.');
  } else {
    setStatus('✅ تم حفظ مفتاح Gemini محلياً على هذا المتصفح.');
  }
  localStorage.setItem(AI_ASSISTANT_API_KEY_STORAGE_KEY, key);
  if (input) { input.value = ''; input.placeholder = 'Gemini API Key محفوظ محلياً'; }
}
...
function aiAssistantToggleSettings() {
  ...
  showAdmin();
  setTimeout(() => {
    document.getElementById('aiAssistantApiKeyAdmin')?.focus();
    document.getElementById('aiAssistantApiKeyAdmin')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
  }, 50);
}
...
async function aiAssistantAsk(prompt) {
  ...
  if (!apiKey) {
    aiAssistantAddMessage('assistant', 'أدخل Gemini API Key من صفحة الإعدادات ⚙️ (قسم "🤖 المساعد الذكي")، ثم اضغط حفظ المفتاح وارجعلي.');
    aiAssistantToggleSettings();
    return;
  }
  ...
  const thinkingIndex = aiAssistantMessages.push({ role: 'assistant', text: aiAssistantPendingImagePart ? 'ثواني... أرسل سؤالك مع الصورة المرفقة للـ AI.' : 'ثواني... أحضّر الجواب.' }) - 1;
  aiAssistantRender();
  try {
...
function initAiAssistant() {
  ...
  aiAssistantRender();
  const keyInput = document.getElementById('aiAssistantApiKeyAdmin');
